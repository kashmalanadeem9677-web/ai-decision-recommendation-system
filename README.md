...
* Education and assessment analysis
...
The key design principle is that the LLM does **not** perform deterministic analysis itself.

Instead:

1. Python evaluates rules and conditions.
2. Matching conditions generate flags.
3. Flags are converted into targeted retrieval queries.
4. PostgreSQL + pgvector retrieves relevant knowledge.
5. Retrieved evidence is passed to the LLM.
6. The LLM generates an explanation or recommendation grounded in that evidence.
...
The system accepts flexible structured data through `DecisionInput`.
...
The deterministic analysis engine supports:
...
means knowledge chunks with a cosine distance greater than `0.30` are excluded.
...
The `/analyze` endpoint accepts retrieval configuration such as:
...
The system can retrieve supporting health knowledge and provide an evidence-grounded decision-support response.

This demonstration is not intended to diagnose a patient or replace professional medical judgment.
...
More generally, recommendation quality depends on:
...
The system therefore exposes rule results and retrieved evidence instead of hiding the intermediate decision pipeline.
...
The system has also been validated with both matching and no-match inputs to ensure that unsupported recommendations are not generated when no configured rule or supporting evidence is available.
